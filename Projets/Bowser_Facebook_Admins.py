...
def contact_has_replied(driver, contact, last_message):
    ...
    messageButton = findElement(driver, "xpath", "//div[@aria-label=\"Message\" and @role=\"button\"]")
    ...
    closeButton = findElement(driver, "xpath", "//div[@aria-label=\"Close chat\" and @role=\"button\"]")
    clickOnElement(driver, closeButton)
...
def send_message(driver, contact, message):
    ...
    """
    ... Code pour envoyer le message dans la plateforme
    """
    try:
        driver.get(contact["url_profile"])
        ...
        chatZone = findElement(driver, "xpath", "//div[@aria-label=\"Aa\" and @role=\"textbox\"]")
...
def contact_has_replied_invitation(driver, contact):
    ...
    try:
        findElement(driver, "xpath", "//div[@aria-label=\"Add Friend\" and @role=\"button\"]")
        has_replied_invitation = False
    except:
        has_replied_invitation = True
...
def send_invitation(driver, url_contact):
    ...
    addFriendButton = findElement(driver, "xpath", "//div[@aria-label=\"Add Friend\" and @role=\"button\"]")
    clickOnElement(driver, addFriendButton)
...
def search_new_contacts(driver, dataBaseTool, logger, platform, browser, id_task_user, current_username, today):
    ...
    """
    list_of_groups = getColumnsFromGoogleSheetByURL(
        "https://docs.google.com/spreadsheets/d/1Q6ZlgjD0buMTVl7VGU_XSSH9w_NT2mJ6gQkhtwVc1g4/edit?usp=sharing")
    #list_of_contacts = getColumnsFromGoogleSheetByURL("https://docs.google.com/spreadsheets/d/1eam0Ki9VwbWRG0Mlv_CTYZj27jEMYJFOR310N0aoaKk/edit?usp=sharing")
    # Sous format [[...], [...], [...]]

    for group in list_of_groups:
        # on charge la page de groupe
        driver.get(group[0])

        # on clique sur "members"

        # on clique sur "See all" s'il existe
        try:
            seeAllButton = driver.find_element_by_xpath("... see all")
            seeAllButton.click()
        except:
            print("ok")

        # on récupère tous les members et le nombre de admin
        members = driver.find_elements_by_xpath("... members")
        nb_admins =  driver.find_element_by_xpath("... nb")

        # on récupère les admins
        admins = members[0:(nb_admins + 1)] # admins = [<div>...</div>, <div>...</div>,...]

        # On scrap les admins
        for admin in admins:
            username = admin.find_element_by_xpath("... /a").text
            url = admin.find_element_by_xpath("... /a").get_attribute("href")


            Sleeping_Bot(2, 3)

def Browser_Cold_Messaging_Facebook_Group_Admins(p_browser, p_taskuser_id, facebook_username, p_quantity_actions, label_log, lock):
    ...
    return cold_messaging_all(driver, lock, logger, p_browser, "facebook", p_taskuser_id, facebook_username, p_quantity_actions)
...
def cold_messaging_all(driver, lock, logger, p_browser, platform, p_taskuser_id, current_username, p_quantity_limit):
    ...
    if len(messageContacts) != 0:
        ...
        for contact in messageContacts:
            ...
            check_answer_in_database =  dataBaseTool.getLinesRequest(
                                                       ...
                                                       (platform, p_taskuser_id, "%received%",
                                                        contact["username"], last_message_id))
            ...
            if res[0]:  # la personne a répondu
                ...
                if not  dataBaseTool.modifyLine("contacts", ["replied", "date_update"], [1, today], {"id": contact["id"]})[
                    "ok"]:
    ...
    *************************************************************************
    ...
    contacts_waiting_for_accept = dataBaseTool.getLinesRequest(
                                                  ...
                                                  "AND actions.type_action LIKE ?",
                                                  (platform, p_taskuser_id, 0, current_username, "%invitation%"))
    ...
    for new_contact in new_contacts:  # new_contacts = [[username, url], [username, url], ....]
        ...
        if need_invitation(driver, new_contact[1]):  # On a besoin de lui envoyer une invitation avant de parler
            ...
            newLineContact = dataBaseTool.addLineWithID("contacts",
                                           ("platform", "username", "url_profile", "id_task_user", "replied_invitation",
    ...
    contacts_to_send = dataBaseTool.getLinesRequest(
                                       ...
                                       "actions.type_action NOT LIKE ? AND actions.id_message IS NOT NULL AND actions.id_social_account=? "
                                       "GROUP BY contacts.id",
                                       (platform, p_taskuser_id, "scrap", current_username, platform, p_taskuser_id,
...
# print(get_next_message_id("message_txt_4A"))
# print(cold_messaging_all("driver", threading.Lock(), "", "Chrome", "freelancer", 250, "@nuksth"))
# print(len(cold_messaging_all("driver", threading.Lock(), "", "Chrome", "freelancer", 250, "@nuksth")))

"""
today = datetime.datetime.now()
last_time = datetime.datetime(2021, 4, 30)
print("{} days".format(get_number_of_days_after_months(last_time, 2)))
delta = today - last_time
print(delta.total_seconds())
"""

# print(addLineWithID(threading.Lock(), "contacts", ("username","platform"), ("haha","facebook")))


# print(get_next_serie_AB(threading.Lock(), "freelancer", 250, "@nuksth"))

# print(get_serie_no_AB(mymodulesteam.GetDetailsTaskUser(250)))


"""
contacts_to_send = getLinesRequest(threading.Lock(), "SELECT contacts.* FROM contacts LEFT JOIN actions ON contacts.id = actions.id_contact "
        "WHERE contacts.platform=? AND contacts.id_task_user=? AND actions.type_action=? AND actions.id_social_account=? GROUP BY contacts.id "
        "EXCEPT SELECT contacts.* FROM contacts INNER JOIN actions ON contacts.id = actions.id_contact "
        "WHERE contacts.platform=? AND contacts.id_task_user=? AND actions.type_action LIKE ? AND "
        "actions.type_action NOT LIKE ? AND actions.id_message IS NOT NULL AND actions.id_social_account=? "
        "GROUP BY contacts.id",
        ("freelancer", 250, "scrap", "@nuksth", "freelancer", 250, "%message%", "%received%", "@nuksth"))
print(contacts_to_send["data"])
print(len(contacts_to_send["data"]))
"""

"""
list_of_contacts = getColumnsFromGoogleSheetByURL(
        "https://docs.google.com/spreadsheets/d/1Q6ZlgjD0buMTVl7VGU_XSSH9w_NT2mJ6gQkhtwVc1g4/edit?usp=sharing")
...
p_quantity_actions_run = 10
label_log_run = ""
lock_run = threading.Lock()

Browser_Cold_Messaging_Facebook_Group_Admins(p_browser_run, p_taskuser_id_run, facebook_username_run, p_quantity_actions_run, label_log_run, lock_run)